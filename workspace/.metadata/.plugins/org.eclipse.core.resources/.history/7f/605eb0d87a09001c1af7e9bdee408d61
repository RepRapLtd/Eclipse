...
public class Triangulation 
{
	...
	private Color3f colour = null;				// The default colour of these objects; this is pushed down to the individual triangles
	...
	public class Triangle 
	{
		 ...
		 * @return
		 */
		private void ShellHedgehogR(ArrayList<Point3D> spines, double flatDihedralAngle)
		{
			Vector3f spine = NonUnitNormal();
			spine.scale(0.5f);
			spines.set(spines.size() - 1, Point3D.add(spines.get(spines.size() - 1), new Point3D(spine)));
			SetVisited();
		 ...
		 * @param out
		 */
		private ArrayList<Point3D> ShellHedgehog(ArrayList<Point3D> spines, double flatDihedralAngle)
		{
...
*/
	...
	private void PrintStatistics()
	{
		...
		ArrayList<Point3D> hedgehog = Hedgehog(0.1);
		double magnitudeSum = 0.0;
		for(int i = 0; i < hedgehog.size(); i++)
		{
			Point3D spine = hedgehog.get(i);
			magnitudeSum += spine.mod();
     ...
     * The hedgehog is a list of normal vectors, one per face, the lengths of which
     * are equal to the areas of the faces.  A face can be just one triangle, or any connected
     * collection of triangles.  The edge of such a collection is decided by flatDihedralAngle:
     * any edge with a bigger dihedral angle is considered to be a boundary of the face.
     * 
     * The normal vectors are sorted by face area, biggest first.
     * 
     * @return
     */
    public ArrayList<Point3D> Hedgehog(double flatDihedralAngle)
    {
    	ArrayList<Point3D> spines = new ArrayList<Point3D>();
		for(int shell = 0; shell < shells.size(); shell++)
		{
			spines.add(new Point3D());
			shells.get(shell).ShellHedgehog(spines, flatDihedralAngle);
		}
		Collections.sort(spines);
    	return spines;
    }
...
//       Triangulation t = new Triangulation("file:///home/ensab/Desktop/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/two-disjoint-cubes.stl", new Color3f(1.0f, 1.0f, 0.0f));
//    	Triangulation t = new Triangulation("file:///home/ensab/Desktop/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/two-overlapping-cubes.stl", new Color3f(1.0f, 1.0f, 0.0f));
//    	Triangulation t = new Triangulation("file:///home/ensab/Desktop/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/hole-enclosed-in-cylinder.stl", new Color3f(1.0f, 1.0f, 0.0f));
//    	Triangulation t = new Triangulation("file:///home/ensab/Desktop/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/two-nonmanifold-cubes.stl", new Color3f(1.0f, 1.0f, 0.0f));
//    	Triangulation t = new Triangulation("file:///home/ensab/Desktop/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/two-nasty-nonmanifold-cubes.stl", new Color3f(1.0f, 1.0f, 0.0f));
    	Triangulation t = new Triangulation("file:///home/ensab/rrlOwncloud/RepRapLtd/Engineering/Software/Eclipse/workspace/polyhedra/554.2-extruder-drive-pneumatic.stl", new Color3f(1.0f, 1.0f, 0.0f));
    	t.PrintStatistics();
