...
package com.reprapltd.triangulation;
...
public class Triangulation 
{
	...
	private boolean debugging = true;           // One day this'll be an execution parameter...	
	...
	private class CornerList
	{
		...
		private boolean Sane()
		{
			...
			if(!result)
			{
				Error("Triangulation.CornerList.Sane(): These should be equal - corners: " + String.valueOf(corners.size()) +
		...
		private boolean StitchUpThisTriangle(Triangle triangle, ArrayList<Triangle> extraTriangles)
		{
			...
			for(int j = 0; j < 3; j++)
			{
				...
				if(neighbour == null)
				{
					Error("Triangulation.CornerList.StitchUpThisTriangle() - triangle with missing neighbour found.", true);
					result = false;
				}
	...
	private class Triangle 
	{
		...
		private int Set()
		{
			...
			for(int i = 0; i < 3; i++)
			{
				if(neighbours[i] != null)
				{
					...
					Error("Triangulation.Triangle.Set(): triangle with null neighbour found.", false);
			}
			return count;
		}
		...
		private int Reset()
		{
			...
			for(int i = 0; i < 3; i++)
			{
				if(neighbours[i] != null)
				{
					...
					Error("Triangulation.Triangle.Reset(): triangle with null neighbour found.", false);
			}
			return count;
		}
		...
		private int Count()
		{
			...
			if(count1 != count0)
				Error("Triangulation.Triangle.Count(): set count (" + String.valueOf(count0) + ") not equal to reset count (" + String.valueOf(count1) + ").", true);
		...
		private void SaveTrianglesR(PrintStream out)
		{
			...
			for(int i = 0; i < 3; i++)
			{
				if(neighbours[i] != null)
				{
					...
					Error("Triangulation.Triangle.SaveTrianglesR(): triangle with null neighbour found.", true);
			}			
		}
	...
	private void BuildFromSTL()
	{
	    ...
	    {
	        Error("Triangulation.Triangulation(): Exception loading STL file from: " + fileLocation, false);
	        e.printStackTrace();
	        System.exit(1);
	    }
		...
		if(scene == null)
		{
			Error("Triangulation.Triangulation(): no triangulation to process", false);
			System.exit(2);
		}
        ...
        if(!cornerList.Sane())
        {
        	Error("Triangulation.Triangulation(): Sane() failed after adding all the triangles.", true);
        	System.exit(3);
        }
	...
	private Triangulation(String location) 
	{
		...
		Error("Triangulation.Triangulation(): file name does not end in .stl or .tri (not case sensitive) - " + fileLocation, true);
		System.exit(3);
	}
	...
	private void Save(String location)
	{
		...
		{
			Error("Triangulation.Save(): Error writing to " + location, true);
			e.printStackTrace();
		}
	}
